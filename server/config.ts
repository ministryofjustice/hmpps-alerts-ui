...
export default {
  ...
  apis: {
    ...
    alertsApi: {
      ...
      healthPath: '/health/ping',
      timeout: {
        response: Number(get('ALERTS_API_TIMEOUT_RESPONSE', 5 * 60 * 1000)),
        deadline: Number(get('ALERTS_API_TIMEOUT_DEADLINE', 5 * 60 * 1000)),
      },
      agent: new AgentConfig(Number(get('ALERTS_API_TIMEOUT_RESPONSE', 5 * 60 * 1000))),
    },
    prisonerSearchApi: {
